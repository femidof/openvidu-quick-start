...
import 'package:video_test/widgets/text_field.dart';
...
class _ConnectPageState extends State<ConnectPage> {
  ...
  Future<void> _readPrefs() async {
    ...
    _uriCtrl.text = const bool.hasEnvironment('URL')
        ...
        : prefs.getString(_storeKeyUri) ??
            'wss://192-168-1-27.openvidu-local.dev:7443';
  ...
  // Future<void> _pre_connect(BuildContext ctx) async {
  //   try {
  //     setState(() {
  //       _busy = true;
  //     });

  //     const serverUrl = 'http://192.168.1.27:6080';

  //     if (serverUrl.isEmpty) {
  //       throw Exception('Server URL is required to fetch the token.');
  //     }

  //     // Perform the API call to fetch the token
  //     final response = await http.get(Uri.parse('$serverUrl/token'));
  ...
  //     print('Data: $data');
  //     if (data['token'] == null) {
  //       throw Exception('Token not found in the server response.');
  //     }
  ...
  //     print('Token fetched successfully: ${_tokenCtrl.text}');
  //   } catch (error) {
  //     print('Error in pre-connect: $error');
